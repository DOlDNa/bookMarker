...
IncSearch.ViewBookmark.prototype.tagsString = function(tags, sep, patternList) {

  if (typeof(tags) == 'string') return tags;

  sep = sep || ' ', l = tags.length;
  if (l != 0) {
    //return '[' + tags.join(']' + sep + '[') + ']';
	var tag = [];
	for (var i=0; i<l; i++)
		tag.push('<a href="#TOP" onclick="d.getElementById(\'search\').value=\''+tags[i]+'\'">'+this.createText(tags[i], patternList)+'<\/a>');
    return tag.join(sep);
  } else {
    return tags.join(sep);
  }
...
{
  var text = ['<a class=link target="_blank" rel="noopener noreferrer" href="', post.url, '">'];

  text.push(this.createText(post.title, patternList));
  text.push('<span class=url>'+this.createText(post.url, patternList)+'<\/span>');
  text.push('<\/a>');
