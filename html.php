...
if (isset($cl))
{
	...
	while ($i < $d)
	{
		...
		echo
		'<ul', ($sort_by_time ? '' : ' draggable=true'), '>',
		'<li><a href="', $uri[$i], '" rel="noopener noreferrer" target="_blank">', h($name[$i]), '</a></li>',
...
'</footer>',
'<script>[].slice.call(document.querySelectorAll("ul")||[]).map((e,i)=>{e.id="l"+i;e.addEventListener("dragstart",ev=>{a=ev.target;ev.dataTransfer.effectAllowed="move";ev.dataTransfer.setData("text/html",e.outerHTML)},false);e.addEventListener("dragover",ev=>{ev.preventDefault()},false);e.addEventListener("drop",ev=>{if(a!==ev.target){ev.target.parentNode.removeChild(a);ev.target.insertAdjacentHTML("beforebegin",ev.dataTransfer.getData("text/html"))}b=[];Array.from(ev.target.parentNode.children,f=>b+=f.lastChild.lastChild.href.split("delete=")[1]+"\n");d(b);return false},false);e.addEventListener("contextmenu",ev=>{ev.preventDefault();u=ev.target.closest("ul"),r=u.lastChild.lastChild;if(r){s=r.href.split("delete=")[1],q=s.split("%2C"),t=document.createElement("input"),t.name="i"+u.id,t.type="text";t1=t.cloneNode(1),t2=t.cloneNode(1),t3=t.cloneNode(1),t.setAttribute("value",decodeURIComponent(q[0])),t1.setAttribute("value",decodeURIComponent(q[1])),t2.setAttribute("value",decodeURIComponent(q[2])),t3.setAttribute("value",decodeURIComponent(q[3]));ev.target.closest("ul").innerHTML=t.outerHTML+t1.outerHTML+t2.outerHTML+t3.outerHTML;[].slice.call(document.querySelectorAll("[name="+t.name+"]")||[]).map((en,ei)=>{en.addEventListener("keydown",evt=>{if(13===evt.keyCode){fetch("./?delete="+s).then(()=>d(f(document.getElementsByName(t.name)[0].value)+","+f(document.getElementsByName(t.name)[1].value)+","+f(document.getElementsByName(t.name)[2].value)+","+f(document.getElementsByName(t.name)[3].value)+"\n"))}})})}return false},false)});function d(a){let b=new FormData();b.append("test",a);fetch("./index.php",{method:"POST",cache:"no-cache",body:b}).then(()=>location.reload())}function f(b){return encodeURIComponent(b.replace(/,/g,"").trim())}</script>',
'</body>',
'</html>';
